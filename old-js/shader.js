...
export class Shader {
    ...
    setMat4(name, value) {
        ...
        if (uniform) this.gl.uniformMatrix4fv(uniform, false, value);
    }

    setInt(name, value) {
        const uniform = this.getUniform(name);
        if (uniform) this.gl.uniform1i(uniform, value);
    }

    setFloat(name, value) {
        const uniform = this.getUniform(name);
        if (uniform) this.gl.uniform1f(uniform, value);
    }

    setVec3(name, value) {
        const uniform = this.getUniform(name);
        if (uniform) this.gl.uniform3fv(uniform, value);
    }
